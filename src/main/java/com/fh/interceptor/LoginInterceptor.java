...
public class LoginInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        String base64LoginUserJson = tokenArr[0];
        ...
        String loginUserJson = new String(Base64.getDecoder().decode(base64LoginUserJson));
        User loginUser = JSONObject.parseObject(loginUserJson, User.class);*/
        ...
        if(!redisTemplate.hasKey("user:"+loginUser.getUuid())){
            JsonUtil.outJson(response, ServerResponse.error(ResponseEnum.TOKEN_IS_EXPIRED));
            return false;
        }

        //给redis中当前登录会员对应的key进行续命操作
        redisTemplate.expire("user:"+loginUser.getUuid(),30, TimeUnit.MINUTES);
